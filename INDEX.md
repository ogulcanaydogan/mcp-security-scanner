...
Current index for the implemented Sprint 1-10K scope, with Sprint 10K stabilization completed on top of `v1.0.32`.
...
- Sprint 9L: done (post-1.0 stabilization hardening with strengthened canonical backend-contract drift detection and deterministic PyPI visibility retry logging under shared release-consistency script)
- Sprint 9M: done (advanced OAuth cache backend v25 with MongoDB KV using PyMongo, env-DSN auth, fixed-schema pre-provisioned document model)
- Sprint 9N: done (post-1.0 stabilization hardening with canonical contract snapshot reuse, callable-map drift guards, and deterministic PyPI visibility diagnostics without runtime behavior changes)
...
- Sprint 10H: done (stabilization-only hardening with deterministic contract total-mismatch annotation and deterministic PyPI visibility check-start diagnostics, runtime behavior unchanged)
- Sprint 10I: done (post-freeze provider discovery gate with objective shortlist scoring and locked next target: Sprint 10J `nats_kv`)
- Sprint 10J: done (advanced OAuth cache backend v32 with `nats_kv`, `nats-py` JetStream KV, env-token auth, pre-provisioned bucket/key model)
- Sprint 10K: done (stabilization-only hardening with deterministic `<missing>` symbol diagnostics for backend-contract drift and ANSI-tolerant PyPI visibility parsing/log normalization, runtime behavior unchanged)
...
- `src/mcp_security_scanner/cli.py`
  ...
  - Advanced persistent cache backend v1:
    ...
    - `backend=openbao_kv` (single secret JSON envelope for OAuth cache entries via OpenBao KV v2 secret path; reuses `vault_*` config surface)
    ...
    - `backend=mysql_kv` (single fixed-schema row JSON envelope for OAuth cache entries via PyMySQL)
    - `backend=mongo_kv` (single fixed-schema document JSON envelope for OAuth cache entries via PyMongo)
    - `backend=dynamodb_kv` (single fixed-schema item JSON envelope for OAuth cache entries via DynamoDB `GetItem`/`UpdateItem`)
    - `backend=s3_object_kv` (single fixed-schema object JSON envelope for OAuth cache entries via S3 `GetObject`/`PutObject`)
    - `backend=sqlite_kv` (single fixed-schema row JSON envelope for OAuth cache entries via sqlite3 `SELECT`/`UPDATE`)
