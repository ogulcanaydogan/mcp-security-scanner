...
### Sprint 8AC (GitLab Group Variables Backend v1)

- OAuth persistent cache backend abstraction expanded again:
  - `auth.cache.backend` now also supports:
    - `gitlab_group_variables`
- GitLab group backend contract:
  - required: `gitlab_group_id`, `gitlab_variable_key`
  - optional: `gitlab_token_env` (default `GITLAB_TOKEN`), `gitlab_api_url` (`https`, default `https://gitlab.com/api/v4`)
  - `gitlab_group_id` is numeric-only in v1
  - pre-provisioned model (scanner does not auto-create missing group variables)
- GitLab group backend behavior:
  - auth uses env token only (`gitlab_token_env` / `GITLAB_TOKEN`)
  - reads/writes JSON envelope in configured GitLab group variable
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only

### Sprint 8AD (v1.0 RC Stabilization Freeze)

- Feature freeze applied for v1.0 RC:
  - no new runtime analyzers, transport modes, or OAuth cache providers were added
  - existing CLI/exit-code/report/auth-cache behavior remains unchanged
- OAuth cache contract hardening:
  - supported backend set is explicitly locked by tests during RC stabilization
  - existing shared invariant matrix remains enforced (`persistent=false` bypass, non-fatal provider bypass, local-only `cache rotate`, compare contract)
- Release guard hardening for RC tags:
...
### Sprint 9D (etcd v3 KV Backend v1)

- OAuth persistent cache backend abstraction expanded:
  - `auth.cache.backend` now also supports:
    - `etcd_kv`
- etcd backend contract:
  - required: `etcd_key`
  - optional: `etcd_api_url` (default `http://127.0.0.1:2379`)
  - optional: `etcd_token_env` (default `ETCD_TOKEN`)
  - auth token is env-only; if env token is missing, client proceeds auth-less
  - pre-provisioned model (scanner does not auto-create missing keys)
- etcd backend behavior:
  - read via `POST /v3/kv/range` with base64 key
  - write via preflight `range` + `POST /v3/kv/put` with base64 key/value
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only
...
### Sprint 9F (GitLab Instance Variables Backend v1)

- OAuth persistent cache backend abstraction expanded:
  - `auth.cache.backend` now also supports:
    - `gitlab_instance_variables`
- GitLab instance backend contract (v1):
  - required: `gitlab_variable_key`
  - optional: `gitlab_token_env` (default `GITLAB_TOKEN`)
  - optional: `gitlab_api_url` (default `https://gitlab.com/api/v4`)
  - forbidden for this backend: `gitlab_project_id`, `gitlab_group_id`, `gitlab_environment_scope`
  - environment scope is not supported in v1 instance mode
  - pre-provisioned model (scanner does not auto-create missing instance variables)
- GitLab instance backend behavior:
  - read via `GET /admin/ci/variables/{key}`
  - write via preflight `GET` + `PUT /admin/ci/variables/{key}`
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only
...
### Sprint 9H (PostgreSQL KV Backend v1)

- OAuth persistent cache backend abstraction expanded:
  - `auth.cache.backend` now also supports:
    - `postgres_kv`
- Postgres backend contract (v1):
  - required: `postgres_cache_key`
...
### Sprint 9I (Post-1.0 Stabilization Hardening)

- No new backend added; sprint scope is stabilization-only.
- OAuth cache contract hardening:
  - backend canonical maps (`_SUPPORTED_OAUTH_CACHE_BACKENDS`, remote spec map, loader map, persister map) remain the single dispatch contract source
  - dispatch resolver coverage is explicitly enforced for every non-local backend
...
- Release hardening consistency:
  - publish-time PyPI visibility verification now reuses the shared `scripts/verify_release_consistency.py` script (`--mode pypi-visibility`)
  - deterministic index flags remain enforced (`--no-cache-dir --index-url https://pypi.org/simple`)
- Runtime behavior remains unchanged:
...
### Sprint 9M (MongoDB KV Backend v1)

- Added OAuth persistent cache backend:
  - `auth.cache.backend` now also supports:
    - `mongo_kv`
- Added config fields for Mongo backend:
  - `mongo_cache_key` (required when `backend=mongo_kv`)
  - `mongo_dsn_env` (optional, default `MONGODB_URI`)
- Mongo backend behavior:
  - fixed schema: `mcp_security_scanner.oauth_cache_store` with fields `cache_key` and `payload_json`
  - read via `find_one({"cache_key": ...})`
  - write via preflight `find_one` + `update_one` on same document key (no create path)
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only
...
### Sprint 9O (DynamoDB KV Backend v1)

- Added OAuth persistent cache backend:
  - `auth.cache.backend` now also supports:
    - `dynamodb_kv`
- Added config field for DynamoDB backend:
  - `dynamodb_cache_key` (required when `backend=dynamodb_kv`)
- DynamoDB backend behavior:
  - fixed schema: table `mcp_oauth_cache_store` with attributes `cache_key` and `payload_json`
  - read via `GetItem` on `cache_key`
  - write via preflight `GetItem` + `UpdateItem` on same item key (no create path)
  - reuses existing AWS routing inputs (`aws_region`, `aws_endpoint_url`) for client construction
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only
...
### Sprint 9Q (S3 Object KV Backend v1)

- Added OAuth persistent cache backend:
  - `auth.cache.backend` now also supports:
    - `s3_object_kv`
- Added config fields for S3 object backend:
  - `s3_bucket` (required when `backend=s3_object_kv`)
  - `s3_object_key` (required when `backend=s3_object_kv`)
- S3 object backend behavior:
  - cache payload stored as JSON envelope in pre-provisioned S3 object (`s3_bucket` + `s3_object_key`)
  - reuses existing AWS routing inputs (`aws_region`, `aws_endpoint_url`) for client construction
  - read via `GetObject` and parse JSON envelope
  - write via preflight `GetObject` + `PutObject` on same object key (no create path)
  - provider/read/write/parse/auth/network errors are non-fatal and bypass persistent layer
- lookup/write order remains unchanged:
  - in-memory -> persistent backend -> refresh grant -> primary grant
- `cache rotate` remains local-backend only
...
### Sprint 9V (SQLite KV Backend v1)

- Added OAuth persistent cache backend:
  - `auth.cache.backend` now also supports:
    - `sqlite_kv`
- Added config fields for SQLite backend:
  - `sqlite_cache_key` (required when `backend=sqlite_kv`)
  - `sqlite_dsn_env` (optional, default `SQLITE_DSN`)
- SQLite backend behavior:
  - fixed schema: table `mcp_oauth_cache_store`, columns `cache_key` and `payload_json`
  - DSN is env-only (`sqlite_dsn_env`), no plaintext DSN in config
...
### Sprint 10F (Post-Freeze Provider Discovery Gate)
...
### Sprint 10J (Post-Freeze Provider Onboarding: `nats_kv`)

- OAuth cache backend surface extended with `nats_kv` (no new CLI flag/command):
  - required: `nats_kv_bucket`, `nats_kv_key`
  - optional: `nats_servers_env` (default `NATS_SERVERS`), `nats_token_env` (default `NATS_TOKEN`)
- Provider runtime model:
  - `nats-py` JetStream KV integration for payload read/write
  - pre-provisioned-only writes preserved (missing key/bucket => write skipped)
